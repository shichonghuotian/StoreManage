 ...
 *
 */
public class WViewControllerFactory {

	
	private static WViewControllerFactory INSTANCE;
	
	public static synchronized WViewControllerFactory getInstance() {
		
		if(INSTANCE == null) {
			
			INSTANCE = new WViewControllerFactory();
		}
	...
	public <T> WViewContext<T> createController(final Class<T> controllerClass) throws Exception {
        ...
        WViewContext<T> context = new WViewContext<T>(parent, controller);
            ...
            String nameByController = controllerClass.getSimpleName()
                    .substring(
                            ...
                            controllerClass.getSimpleName().length()
                                    - "Controller".length())
                    + ".fxml";
            if (WFxUtils.canAccess(controllerClass, nameByController)) {
